...
Add explicit command paths for controlling an agent session. Commands should flow through application use cases and an agent runner port, with pi-specific execution hidden behind an infrastructure adapter.
...
- Add an `AgentRunner` port or equivalent functional port in the domain/application boundary.
...
- Agent commands flow through UI/API -> application use case -> agent runner port -> infrastructure adapter.
