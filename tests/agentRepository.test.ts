...
describe("createStubAgentRepository", () => {
  test("starts an agent session and publishes normalized startup events", async () => {
    ...
    scenario.then.eventWasPublished({ type: "agent.output.appended", payload: { sessionId: "session-1", text: "Stub agent session started." } });
  ...
  test("publishes normalized runtime events when the agent receives input", async () => {
    ...
    scenario.then.eventWasPublished({ type: "agent.output.appended", payload: { sessionId: "session-2", text: "Stub agent received input: next step" } });
  ...
  test("stops an agent session as cancellation", async () => {
    ...
    scenario.then.eventWasPublished({ type: "agent.output.appended", payload: { sessionId: "session-3", text: "Stub agent session stopped." } });
